-- Complete Database Migration: Add all missing columns and update RPC functions
-- Date: 2026-09-13
-- Run this entire script in Supabase SQL Editor

BEGIN;

-- 1. Ensure columns on tpi_job_codes
ALTER TABLE public.tpi_job_codes
  ADD COLUMN IF NOT EXISTS billing_normal_rate numeric(12,2),
  ADD COLUMN IF NOT EXISTS billing_skilled_rate numeric(12,2);

-- 2. Ensure columns on tpi_shift_days
ALTER TABLE public.tpi_shift_days
  ADD COLUMN IF NOT EXISTS is_holiday boolean NOT NULL DEFAULT false;

-- 3. Ensure all columns on tpi_shift_entries
...
-- 4. Update tpi_save_shift_day RPC (no FOR SHARE on outer join)
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(p_entries) AS x(
    ...
    VALUES(
      p_factory, p_date, item.employee_id, item.shift_index, item.job_id,
      tier, amount, job.code,
      v_is_half, v_hours, v_ot_hours, v_ot_pay,
      v_is_holiday, v_is_ot_before, item.ot_job_id, item.ot_job_code_snapshot,
      v_is_pending
    )
...
-- 5. Create tpi_preassign_shifts RPC
