 ...
 return <TpiDialog title="ฐานข้อมูลรหัสงาน" onClose={onClose}>
  {editing ? <JobForm key={editing.id} job={editing} onSave={onSave} onClose={()=>setEditing(null)}/> : <div className="tpi-dialog-body">
   ...
   {error&&<p role="alert">{error}</p>}
    <div className="tpi-job-table"><table><thead><tr><th>รหัสงาน / แผนก</th><th>ยอดเต็ม</th><th>ตั้งเบิกปกติ</th><th>ตั้งเบิกฝีมือ</th><th>จ่ายปกติ</th><th>จ่ายฝีมือ</th><th>ส่วนต่างปกติ</th><th>ส่วนต่างฝีมือ</th><th>หมดอายุ</th><th>สถานะ</th><th/></tr></thead><tbody>{jobs.filter(j=>`${j.code} ${j.department} ${j.description}`.toLowerCase().includes(search.toLowerCase())).map(j=>{const bNormal=j.billing_normal_rate??j.billing_rate??null;const bSkilled=j.billing_skilled_rate??null;return <tr key={j.id}><td><strong>{j.code}</strong><small>{j.department}</small><small>{j.description}</small></td><td>{j.quota}</td><td style={{ color: '#1e40af', fontWeight: 600 }}>{bNormal ? `฿${bNormal.toLocaleString()}` : '—'}</td><td style={{ color: '#1e40af', fontWeight: 600 }}>{bSkilled ? `฿${bSkilled.toLocaleString()}` : '—'}</td><td>฿{j.normal_rate.toLocaleString()}</td><td>{j.skilled_rate ? `฿${j.skilled_rate.toLocaleString()}` : '—'}</td><td style={{ color: bNormal ? (bNormal - j.normal_rate >= 0 ? '#15803d' : '#dc2626') : 'inherit', fontWeight: 600 }}>{bNormal ? `+฿${(bNormal - j.normal_rate).toLocaleString()}` : '—'}</td><td style={{ color: bSkilled && j.skilled_rate ? (bSkilled - j.skilled_rate >= 0 ? '#15803d' : '#dc2626') : 'inherit', fontWeight: 600 }}>{bSkilled && j.skilled_rate ? `+฿${(bSkilled - j.skilled_rate).toLocaleString()}` : '—'}</td><td>{j.expires_on ? formatThaiDateShort(j.expires_on) : 'ไม่กำหนด'}</td><td>{!j.active?'ปิดใช้งาน':j.expires_on && j.expires_on < localDate()?'หมดอายุ':'ใช้งาน'}</td><td><button className="vk-btn" onClick={()=>setEditing(j)}>แก้ไข {j.code}</button></td></tr>})}</tbody></table></div>
    </div>}
  </TpiDialog>
}
function JobForm({job,onSave,onClose}:{job:Job;onSave:(j:Job)=>Promise<void>;onClose:()=>void}) {
 ...
 <label>ตั้งเบิกปกติ TPI (บาท/กะ)<input className="vk-input" type="number" min="0" step="0.01" placeholder="เช่น 420" value={form.billing_normal_rate ?? form.billing_rate ?? ''} onChange={e=>patch({billing_normal_rate:e.target.value===''?null:Number(e.target.value),billing_rate:e.target.value===''?null:Number(e.target.value)})}/></label>
 <label>ตั้งเบิกฝีมือ TPI (บาท/กะ)<input className="vk-input" type="number" min="0" step="0.01" placeholder="เช่น 480" value={form.billing_skilled_rate ?? ''} onChange={e=>patch({billing_skilled_rate:e.target.value===''?null:Number(e.target.value)})}/></label>
 <label>ค่าแรงปกติจ่ายคนงาน (บาท/กะ)<input className="vk-input" required type="number" min="0" step="0.01" value={form.normal_rate} onChange={e=>patch({normal_rate:Number(e.target.value)})}/></label>
 <label>ค่าแรงฝีมือจ่ายคนงาน (บาท/กะ)<input className="vk-input" type="number" min="0" step="0.01" placeholder="ยังไม่กำหนด" value={form.skilled_rate??''} onChange={e=>patch({skilled_rate:e.target.value===''?null:Number(e.target.value)})}/></label>
