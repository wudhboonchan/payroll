...
export const PaySlipPreview = forwardRef<HTMLDivElement, PaySlipPreviewProps>(({ data }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className="bg-white font-sans text-slate-800 mx-auto box-border w-full max-w-[850px] min-w-[650px] print:w-[190mm] print:min-w-0 p-8 md:p-10 print:p-0 rounded-2xl shadow-xl print:shadow-none border border-slate-100 print:border-none relative overflow-hidden print:overflow-visible"
    >
      {/* Decorative Top Accent for Screen */}
      <div className="absolute top-0 left-0 w-full h-2 bg-[#1D9E75] print:hidden" />

      {/* ===== HEADER ===== */}
      <div className="flex justify-between items-start mb-8 print:mb-5">
        ...
        <div className="text-right flex flex-col items-end">
          <div className="bg-[#1D9E75]/10 text-[#1D9E75] print:bg-transparent print:text-slate-800 border border-[#1D9E75]/20 print:border-slate-800 px-4 py-1.5 rounded-full text-sm print:text-xs font-bold tracking-wide">
            ใบสลิปเงินเดือน
          </div>
          <p className="text-[10px] md:text-xs print:text-[9px] text-slate-400 mt-4 md:mt-5 print:mt-3 font-medium uppercase tracking-wider">จัดทำเมื่อ</p>
          <p className="text-xs md:text-sm print:text-[10px] font-semibold text-slate-700">{generatedAt}</p>
        </div>
      </div>
      ...
      <div className="border border-slate-200 rounded-xl print:border-slate-800 print:border-2 print:rounded-none overflow-hidden">
        ...
        <div className="grid grid-cols-2 bg-slate-100 border-b border-slate-200 print:border-slate-800 print:bg-slate-100 divide-x divide-slate-200 print:divide-slate-800">
          <div className="px-5 py-3 md:py-4 print:px-3 print:py-2 text-xs md:text-sm print:text-[11px] font-bold text-slate-700 uppercase tracking-wider text-center">
            รายได้ (Earnings)
          </div>
          <div className="px-5 py-3 md:py-4 print:px-3 print:py-2 text-xs md:text-sm print:text-[11px] font-bold text-slate-700 uppercase tracking-wider text-center">
        ...
        <div className="grid grid-cols-2 divide-x divide-slate-200 print:divide-slate-800 bg-white">
          <div className="p-5 md:p-6 print:p-3 space-y-3 print:space-y-2 align-top min-h-[140px] print:min-h-[100px]">
        ...
        <div className="grid grid-cols-2 divide-x divide-slate-200 print:divide-slate-800 bg-slate-50 border-t border-slate-200 print:border-slate-800">
          <div className="px-5 py-3 md:py-4 print:px-3 print:py-2 flex justify-between items-center text-sm print:text-xs font-bold text-slate-700">
            <span>รวมรายได้</span>
      ...
      {/* ===== FOOTER INFO & NET PAY ===== */}
      <div className="mt-6 print:mt-4 flex flex-col md:flex-row print:flex-row gap-4 print:gap-4 items-stretch">
        
        {/* Payment Details */}
        <div className="flex-1 bg-slate-50 border border-slate-100 print:border-slate-800 print:border-2 rounded-xl print:rounded-none p-5 md:p-6 print:p-3 flex flex-col justify-center">
          <h3 className="text-[10px] md:text-xs print:text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-4 md:mb-5 print:mb-2 flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-[#1D9E75] print:hidden" />
            ข้อมูลการชำระเงิน
          </h3>
          <div className="space-y-2.5 print:space-y-1.5 text-xs md:text-sm print:text-[11px] text-slate-700">
            <div className="flex items-center">
              <span className="w-24 md:w-28 print:w-20 font-medium">วิธีการรับเงิน:</span>
              <span className="font-bold text-slate-900">{data.payment_method === 'cash' ? 'เงินสด' : 'โอนบัญชีธนาคาร'}</span>
            </div>
            {data.payment_method === 'bank_transfer' && (
              <>
                <div className="flex items-center">
                  <span className="w-24 md:w-28 print:w-20 font-medium">ธนาคาร:</span>
                  <span className="font-bold text-slate-900">{data.bank_name || '—'}</span>
                </div>
                <div className="flex items-center">
                  <span className="w-24 md:w-28 print:w-20 font-medium">เลขที่บัญชี:</span>
                  <span className="font-bold text-slate-900 tracking-wider">{data.bank_account || '—'}</span>
                </div>
              </>
            )}
          </div>
        ...
        {/* Net Pay Focus */}
        <div className="bg-gradient-to-br from-[#1D9E75] to-[#116e50] text-white print:from-transparent print:to-transparent print:bg-white print:text-slate-900 print:border-2 print:border-slate-800 rounded-xl print:rounded-none p-6 md:p-8 print:p-4 flex flex-col items-center justify-center min-w-[280px] print:min-w-[180px] shadow-lg shadow-emerald-900/20 print:shadow-none">
          <p className="text-xs md:text-sm print:text-[11px] text-emerald-100 print:text-slate-500 font-medium uppercase tracking-wider mb-2 print:mb-1">
            เงินได้สุทธิ (Net Pay)
          </p>
          <div className="flex items-baseline gap-2">
            <span className="text-4xl md:text-5xl print:text-3xl font-black tracking-tight">{formatThaiCurrency(data.net_pay)}</span>
            <span className="text-base md:text-lg print:text-xs text-emerald-100 print:text-slate-500 font-medium">บาท</span>
          </div>
        </div>
