-- Migration Phase 3: Add Half-Shift / Partial-Hour support to TPI Shift Entries
BEGIN;

-- Add columns to tpi_shift_entries if not present
...
-- Update tpi_save_shift_day RPC to support is_half_shift and actual_hours
CREATE OR REPLACE FUNCTION public.tpi_save_shift_day(p_factory uuid, p_date date, p_revision integer, p_entries jsonb) 
RETURNS integer
...
BEGIN
  ...
  IF EXISTS(
    SELECT 1 FROM jsonb_to_recordset(p_entries) AS r(employee_id uuid, shift_index integer, job_id uuid, is_half_shift boolean, actual_hours numeric)
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(p_entries) AS r(employee_id uuid, shift_index integer, job_id uuid, is_half_shift boolean, actual_hours numeric) LOOP
    ...
    -- If assignment already exists with same properties, retain snapshots
    IF EXISTS(
      SELECT 1 FROM tpi_shift_entries 
      WHERE factory_id=p_factory AND work_date=p_date AND employee_id=item.employee_id 
        AND shift_index=item.shift_index AND job_id=item.job_id AND is_half_shift=v_is_half
    ) THEN 
      CONTINUE; 
    END IF;
    ...
    INSERT INTO tpi_shift_entries(factory_id, work_date, employee_id, shift_index, job_id, rate_tier, rate_snapshot, job_code_snapshot, is_half_shift, actual_hours)
    VALUES(p_factory, p_date, item.employee_id, item.shift_index, item.job_id, tier, amount, job.code, v_is_half, v_hours)
  ...
  SET revision=revision+1, updated_at=clock_timestamp(), updated_by=auth.uid() 
  WHERE factory_id=p_factory AND work_date=p_date;
...
REVOKE ALL ON FUNCTION public.tpi_save_shift_day(uuid,date,integer,jsonb) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.tpi_save_shift_day(uuid,date,integer,jsonb) TO authenticated;
