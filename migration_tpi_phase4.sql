-- Migration Phase 4: Add OT support to TPI Shift Entries (Clerk 8h 1.5x & Worker Roof Climbing OT 1.5x/hr)
BEGIN;

-- Add OT columns to tpi_shift_entries if not present
...
-- Update tpi_save_shift_day RPC to support is_half_shift, actual_hours, ot_hours, and ot_pay
...
BEGIN
  ...
  IF EXISTS(
    SELECT 1 FROM jsonb_to_recordset(p_entries) AS r(employee_id uuid, shift_index integer, job_id uuid, is_half_shift boolean, actual_hours numeric, ot_hours numeric, ot_pay numeric)
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(p_entries) AS r(employee_id uuid, shift_index integer, job_id uuid, is_half_shift boolean, actual_hours numeric, ot_hours numeric, ot_pay numeric) LOOP
    ...
    INSERT INTO tpi_shift_entries(factory_id, work_date, employee_id, shift_index, job_id, rate_tier, rate_snapshot, job_code_snapshot, is_half_shift, actual_hours, ot_hours, ot_pay)
    VALUES(p_factory, p_date, item.employee_id, item.shift_index, item.job_id, tier, amount, job.code, v_is_half, v_hours, v_ot_hours, v_ot_pay)
