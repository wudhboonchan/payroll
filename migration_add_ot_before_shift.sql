-- Migration: Add is_ot_before_shift, ot_job_id, ot_job_code_snapshot to tpi_shift_entries and update tpi_save_shift_day RPC
-- Date: 2026-09-13

BEGIN;

-- 1. Add columns to tpi_shift_entries if not exists
ALTER TABLE public.tpi_shift_entries
ADD COLUMN IF NOT EXISTS is_ot_before_shift boolean NOT NULL DEFAULT false,
ADD COLUMN IF NOT EXISTS ot_job_id uuid REFERENCES public.tpi_job_codes(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS ot_job_code_snapshot text;

-- 2. Update tpi_save_shift_day RPC to handle is_ot_before_shift and ot_job
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(p_entries) AS x(
    ...
    INSERT INTO tpi_shift_entries(factory_id, work_date, employee_id, shift_index, job_id, rate_tier, rate_snapshot, job_code_snapshot, is_half_shift, actual_hours, ot_hours, ot_pay, is_holiday_ot, is_ot_before_shift, ot_job_id, ot_job_code_snapshot)
    VALUES(p_factory, p_date, item.employee_id, item.shift_index, item.job_id, tier, amount, job.code, v_is_half, v_hours, v_ot_hours, v_ot_pay, v_is_holiday, v_is_ot_before, item.ot_job_id, item.ot_job_code_snapshot)
