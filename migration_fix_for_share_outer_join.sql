-- Migration Fix: Remove FOR SHARE from outer join query in tpi_save_shift_day RPC
-- Fixes PostgreSQL error: "FOR SHARE cannot be applied to the nullable side of an outer join"
-- Date: 2026-09-13
...
BEGIN
  ...
  FOR item IN SELECT * FROM jsonb_to_recordset(p_entries) AS x(
    ...
    -- Check if employee has skilled wage tier (without FOR SHARE on outer join)
    SELECT 
      CASE 
